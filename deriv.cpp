...
void calc_deriv_1(dv& x1, const dv& x)
{
   ...
   x1[0] = (-x[2] + 8.0*x[1] - 8.0*x[len-1] + x[len-2])/12.0;
   x1[1] = (-x[3] + 8.0*x[2] - 8.0*x[0]     + x[len-1])/12.0;
   for (unsigned int i=2; i<len-2; ++i)
   {
      x1[i] = (-x[i+2] + 8.0*x[i+1] - 8.0*x[i-1] + x[i-2])/12.0;
   }
   x1[len-2] = (-x[0] + 8.0*x[len-1] - 8.0*x[len-3] + x[len-4])/12.0;
   x1[len-1] = (-x[1] + 8.0*x[0]     - 8.0*x[len-2] + x[len-3])/12.0;
} // end of calc_deriv_1
...
void calc_deriv_2(dv& x2, const dv& x)
{
   ...
   x2[0] = (-x[2] + 16.0*x[1] - 30*x[0] + 16.0*x[len-1] - x[len-2])/12.0;
   x2[1] = (-x[3] + 16.0*x[2] - 30*x[1] + 16.0*x[0]     - x[len-1])/12.0;
   for (unsigned int i=2; i<len-2; ++i)
   {
      x2[i] = (-x[i+2] + 16.0*x[i+1] - 30*x[i] + 16.0*x[i-1] - x[i-2])/12.0;
   }
   x2[len-2] = (-x[0] + 16.0*x[len-1] - 30*x[len-2] + 16.0*x[len-3] - x[len-4])/12.0;
   x2[len-1] = (-x[1] + 16.0*x[0]     - 30*x[len-1] + 16.0*x[len-2] - x[len-3])/12.0;
} // end of calc_deriv_2
