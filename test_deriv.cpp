...
void calc_x3(dv& x2, const dv& x)
{
   ...
   x2[1] = (    -x[4] +  6.0*x[3] - 12.0*x[2] + 10.0*x[1] - 3.0*x[0])*0.5;

   x2[len-2] = (    x[len-5] -  6.0*x[len-4] + 12.0*x[len-3] - 10.0*x[len-2] + 3.0*x[len-1])*0.5;
   x2[len-1] = (3.0*x[len-5] - 14.0*x[len-4] + 24.0*x[len-3] - 18.0*x[len-2] + 5.0*x[len-1])*0.5;
